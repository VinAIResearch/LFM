...
def sample_and_test(args):
    ...
    if args.dataset == "coco":
        dataset = CocoImagesAndCaptionsValidation(
    ...
    cond_ckpt = torch.load(
        "./saved_info/latent_flow_mask2image/{}/{}/cond_stage_model_{}.pth".format(
            args.dataset, args.exp, args.epoch_id
        ),
        map_location=device,
    )
    ...
    ckpt = torch.load(
        "./saved_info/latent_flow_mask2image/{}/{}/model_{}.pth".format(
            args.dataset, args.exp, args.epoch_id
        ),
        map_location=device,
    )
    ...
    for i, (image, segmentation) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            model_cond.cond = c
            z_0 = torch.randn(
                image.size(0), 4, args.image_size // 8, args.image_size // 8
            ).to(device)
            ...
            for j, x in enumerate(fake_image):
                ...
                torchvision.utils.save_image(
                    x,
                    "./mask2image_generated_samples/{}/{}.jpg".format(
                        args.dataset, index
                    ),
                    normalized=True,
                )
...
if __name__ == "__main__":
    ...
    # sampling argument
    parser.add_argument(
        "--atol", type=float, default=1e-5, help="absolute tolerance error"
    )
    parser.add_argument(
        "--rtol", type=float, default=1e-5, help="absolute tolerance error"
    )
    parser.add_argument(
        "--method",
        type=str,
