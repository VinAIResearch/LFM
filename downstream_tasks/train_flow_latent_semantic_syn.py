...
import torchvision
from datasets_prep.coco import (
    CocoImagesAndCaptionsTrain,
...
def train(rank, gpu, args):
    ...
    if args.dataset == "coco":
        dataset = CocoImagesAndCaptionsTrain(
            size=256, onehot_segmentation=True, use_stuffthing=True
        )
        num_cls = 182
    ...
    cond_stage_model = SpatialRescaler(
        n_stages=3, in_channels=num_cls, out_channels=4, multiplier=0.5
    ).to(device)
    ...
    optimizer = optim.AdamW(
        list(model.parameters()) + list(cond_stage_model.parameters()),
        lr=args.lr,
        weight_decay=0.0,
    )
    ...
    cond_stage_model = nn.parallel.DistributedDataParallel(
        cond_stage_model, device_ids=[gpu], find_unused_parameters=False
    )
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (image, segmentation) in enumerate(data_loader):
            segmentation = torch.nn.functional.one_hot(segmentation, num_cls).permute(
                0, 3, 1, 2
            )
        ...
        if rank == 0:
            ...
            torchvision.utils.save_image(
                to_rgb(seg),
                os.path.join(exp_path, "image_epoch_seg_{}.png".format(epoch)),
                normalize=True,
            )
            torchvision.utils.save_image(
                fake_image,
            ...
            torchvision.utils.save_image(
                image[:4],
                os.path.join(exp_path, "image_epoch_{}_gt.png".format(epoch)),
                normalize=True,
            )
            ...
            if args.save_content:
                if epoch % args.save_content_every == 0:
                    ...
                    content = {
                        ...
                        "cond_stage_model_dict": cond_stage_model.state_dict(),
                    }
            ...
            if epoch % args.save_ckpt_every == 0:
                ...
                torch.save(
                    cond_stage_model.state_dict(),
                    os.path.join(exp_path, "cond_stage_model_{}.pth".format(epoch)),
                )
                if args.use_ema:
                    optimizer.swap_parameters_with_ema(store_params_in_ema=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_in_channels", type=int, default=8, help="in channel image"
    )
    parser.add_argument(
        "--num_out_channels", type=int, default=4, help="in channel image"
    )
    ...
    parser.add_argument(
        "--ch_mult", nargs="+", type=int, default=(1, 2, 3, 4), help="channel mult"
    )
