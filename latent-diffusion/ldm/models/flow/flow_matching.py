...
class FlowMatching(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.model = FlowMatchingWrapper(unet_config, conditioning_key)
    ...
    def forward(self, batch):
        x_1 = self.get_input(batch, self.first_stage_key)
...
class LatentFlowMatching(FlowMatching):
    ...
    def forward(self, batch):
        x_1, c = self.get_input(batch, self.first_stage_key)
    ...
    def apply_model(self, t, v_t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            fold, unfold, normalization, weighting = self.get_fold_unfold(v_t, ks, stride)

            z = unfold(v_t)  # (bn, nc * prod(**ks), L)
            # Reshape to img shape
            z = z.view((z.shape[0], -1, ks[0], ks[1], z.shape[-1]))  # (bn, nc, ks[0], ks[1], L )
            ...
            output_list = [self.model(t, z_list[i], **cond_list[i]) for i in range(z.shape[-1])]
...
class FlowMatchingWrapper(pl.LightningModule):
    def __init__(self, fm_model_config, conditioning_key):
        super().__init__()
        self.flowmatching_model = instantiate_from_config(fm_model_config)
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm']

    def forward(self, t, x, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            out = self.flowmatching_model(t, x)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.flowmatching_model(t, xc)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            out = self.flowmatching_model(t, x, context=cc)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.flowmatching_model(t, xc, context=cc)
        elif self.conditioning_key == 'adm':
            cc = c_crossattn[0]
            out = self.flowmatching_model(t, x, y=cc)
        else:
            raise NotImplementedError()
        return out


class Layout2ImgFlowMatching(LatentFlowMatching):
