...
class PairwiseScore(EvaluatorScore, ABC):
    ...
    def get_value(self, groups=None, states=None):
        ...
        """
        individual_values = (
            torch.cat(states, dim=-1).reshape(-1).cpu().numpy() if states is not None else self.individual_values
        )
...
class FIDScore(EvaluatorScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        activations_pred, activations_target = (
            zip(*states) if states is not None else (self.activations_pred, self.activations_target)
        )
...
class SegmentationAwareScore(EvaluatorScore):
    ...
    def forward(self, pred_batch, target_batch, mask):
        pred_segm_flat = (
            self.segm_network.predict(pred_batch)[0].view(pred_batch.shape[0], -1).long().detach().cpu().numpy()
        )
        target_segm_flat = (
            self.segm_network.predict(target_batch)[0].view(pred_batch.shape[0], -1).long().detach().cpu().numpy()
        )
...
class SegmentationClassStats(SegmentationAwarePairwiseScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        pred_class_freq_diff = (pred_class_freq_by_image_mask - target_class_freq_by_image_mask).sum(0) / (
            target_class_freq_by_image_mask.sum(0) + 1e-3
        )
        ...
        for label, index in grouping.items():
            ...
            group_target_class_freq_by_image_total_marginal = group_target_class_freq_by_image_total.sum(0).astype(
                "float32"
            )
            group_target_class_freq_by_image_total_marginal /= group_target_class_freq_by_image_total_marginal.sum()

            group_target_class_freq_by_image_mask_marginal = group_target_class_freq_by_image_mask.sum(0).astype(
                "float32"
            )
            ...
            group_pred_class_freq_diff = (
                group_pred_class_freq_by_image_mask - group_target_class_freq_by_image_mask
            ...
            if len(index) > 1:
                ...
                group_results[label] = {
                    ...
                    **self.distribute_fid_to_classes(
                        group_class_freq, group_activations_pred, group_activations_target
                    ),
                }
            else:
