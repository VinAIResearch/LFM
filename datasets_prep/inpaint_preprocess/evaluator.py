...
import tqdm
from datasets_prep.inpaint_preprocess.utils import move_to_device
...
class InpaintingEvaluator:
    ...
    def _get_bin_edges(self):
        ...
        for idx_bin in range(self.bins):
            start_percent, end_percent = round(100 * bin_edges[idx_bin], num_digits), round(
                100 * bin_edges[idx_bin + 1], num_digits
            )
                ...
                for batch in tqdm.auto.tqdm(self.dataloader, desc=score_name, leave=False):
                    ...
                    if self.clamp_image_range is not None:
                        image_batch = torch.clamp(
                            image_batch, min=self.clamp_image_range[0], max=self.clamp_image_range[1]
                        )
                    if model is None:
                        assert (
                            "inpainted" in batch
...
class InpaintingEvaluatorOnline(nn.Module):
    def __init__(
        ...
        for idx_bin in range(self.bins_num):
            start_percent, end_percent = round(100 * self.bin_edges[idx_bin], num_digits), round(
                100 * self.bin_edges[idx_bin + 1], num_digits
            )
            start_percent = "{:.{n}f}".format(start_percent, n=num_digits)
            end_percent = "{:.{n}f}".format(end_percent, n=num_digits)
